...
export function TtsButton({ text }: TtsButtonProps) {
  ...
  const handleClick = useCallback(async () => {
    ...
    if (cachedUrl) {
      ...
      audio.onpause = () => { if (!audio.ended) setState("idle"); };
      audio.play();
      setState("playing");
      return;
    }
  ...
  const Icon = state === "loading" ? Loader2 : state === "playing" ? Pause : state === "error" ? AlertCircle : Volume2;
  ...
  return (
    <button
      ...
      title={state === "playing" ? "Pause" : state === "error" ? "TTS failed" : "Read aloud"}
    >
